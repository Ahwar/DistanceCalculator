...
project = rf.workspace().project("car-make-and-model-recognition")
model = project.version(2).model

project_color = rf.workspace().project("car-colors-1smyc")
...
for image_path in globbed_files:
    ...
    response_json = model.predict(image_path, confidence=19, overlap=50).json()
    # print(response_json)

    for object in response_json["predictions"]:

        object_class = str(object["class"])
        object_class_text_size = cv2.getTextSize(
            object_class, font, font_scale, font_thickness
        )
        ...
        if not os.path.exists("cropped"):
            os.makedirs("cropped")
        image_cropped_path = (
            "cropped/cropped_" + object_class + "_" + str(counter) + ".jpg"
        )
        ...
        confidence_font_start_point = (
            int(x0) + object_class_text_size[0][0],
            int(y0) - 10,
        )
        ...
        image_drawn = cv2.rectangle(
            image_clean, box_start_point, box_end_point, box_color, box_thickness
        )

        image_drawn_blk = cv2.rectangle(
            blk, box_start_point, box_end_point, box_color, box_thickness
        )

        image_drawn = cv2.putText(
            image_drawn,
            object_class,
            class_font_start_point,
            font,
            font_scale,
            font_color,
            font_thickness,
        ...
        if object_class == "Hyundai Azera Sedan 2012":
        ...
        if object_class == "Scion xD Hatchback 2012":
    ...
    try:
        if elantra_width and rio_width:
            ...
            if x_calc_1 == min_pixel_distance:
                ...
                estimated_distance_inches = round(
                    min_pixel_distance / pixel_to_inches, 2
                )
                distance_start_point = (rio_x_min, rio_y_max - rio_half_height)
                distance_end_point = (
                    elantra_x_min,
                    elantra_y_max - elantra_half_height,
                )
                distance_text_point = (
                    rio_x_min + int(x_calc_1 / 5),
                    (elantra_y_max - elantra_half_height) - 10,
                )
                ...
                distance_text_point = (
                    rio_x_max + int(x_calc_2 / 5),
                    (elantra_y_max - elantra_half_height) - 20,
                )
            ...
            image_drawn = cv2.putText(
                image_drawn,
                "~" + str(estimated_distance_inches) + "in.",
                distance_text_point,
                font,
                font_scale,
                font_color,
